...
| **cacheMode**         | `Function` | Customize how tapes are loaded and saved based on the incoming request. [More info](#cache-mode)                                      | `() => "cache")`            |
...
If the request or response have a JSON _content-type_, their body will be pretty printed as an object in the tape for easier readability.  
...
By default, Talkback is designed to replay a request from a saved tape if one exists, like a cache. However, you may have a use-case for customizing this behavior on a request-by-request basis. Setting the `cacheMode` option allows you to do this.

The function takes the incoming `Request` object as input, and must return one of the following strings:

- `cache`: Replay a saved tape if one exists, otherwise save a new tape
- `no-cache`: Don't replay a saved tape, always save a new tape for every request
- `pass-thru`: Don't replay a saved tape, and don't save a new tape
